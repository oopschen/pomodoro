            ...
            match evts.get(i) {
                ...
                Some(evt) => {
                    match dispatch_event(
                        time_args, &evt,
                        &mut holder_arr, &mut avail_index,
...
fn dispatch_event(time_args: PTime, evt: &Event, 
                  ...
                  avails: &mut VecDeque<usize>, listener: &TcpListener,
                  maxp: usize, notify_progs: &str) -> PollAction {
    // if token is server
    //  is reable:
    //      write help message
    //      get next index
    ...
    match token {
        ...
        y if y > maxp => {
            if let Some(ref hdl) = holders[y-maxp] {
                deal_timer_token(hdl, y, notify_progs)
            } else {
                PollAction::EXIT(y)
            }
        },
...
fn deal_timer_token(holder: &PomodoroHolder, inx: usize, notify_progs: &str) -> PollAction {
    // get current status
    // assemble status text, write to buf
    // call notify program
    let notify_progs_clone = match notify_progs {
